...
// ==========================================
// Beam Search (without hash deduplication)
// ==========================================
...
struct Candidate {
    ...
    Candidate(Action action, Evaluator evaluator, int parent)
        : action(action), evaluator(evaluator), parent(parent) {}
};
...
class State {
  public:
    ...
    void expand(const Evaluator &evaluator, int parent, Selector &selector) {
        // TODO
    }
...
class Tree {
  public:
    ...
    void dfs(Selector &selector) {
        if (curr_tour_.empty()) {
            // 最初のターン
            Evaluator evaluator = state_.make_initial_node();
            state_.expand(evaluator, 0, selector);
            return;
        }
        ...
        for (auto [leaf_index, action] : curr_tour_) {
            if (leaf_index >= 0) {
                ...
                Evaluator &evaluator = leaves_[leaf_index];
                state_.expand(evaluator, leaf_index, selector);
    ...
    void update(const vector<Candidate> &candidates) {
        ...
        if (curr_tour_.empty()) {
            ...
            for (const Candidate &candidate : candidates) {
                ...
                leaves_.push_back(candidate.evaluator);
            }
            return;
        }
        ...
        for (const Candidate &candidate : candidates) {
            buckets_[candidate.parent].push_back(
                {candidate.action, candidate.evaluator});
        }
  ...
  private:
    ...
    vector<Evaluator> leaves_;
    vector<vector<pair<Action, Evaluator>>> buckets_;
